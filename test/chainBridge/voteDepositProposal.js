...
contract('Bridge - [voteDepositProposal with relayerThreshold > 1]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        tokenID = Ethers.utils.toUtf8Bytes(`0${destinationChainID}${DestinationERC20MintableInstance.address}`);

        data = '0x' +
            Ethers.utils.hexZeroPad(Ethers.utils.hexlify(tokenID), 32).substr(2) +
