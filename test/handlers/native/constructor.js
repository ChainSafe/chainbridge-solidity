...
contract('NativeAssetHandler - [constructor]', async () => {
    ...
    it('[sanity] contract should be deployed successfully', async () => {
        TruffleAssert.passes(await NativeAssetHandlerContract.new(BridgeInstance.address, initialResourceIDs, initialContractAddresses, burnableContractAddresses));
    });

    it('initialResourceIDs should be parsed correctly and corresponding resourceID mappings should have expected values', async () => {
        const NativeAssetHandlerInstance = await NativeAssetHandlerContract.new(BridgeInstance.address, initialResourceIDs, initialContractAddresses, burnableContractAddresses);
        
        for (const resourceID of initialResourceIDs) {
            const tokenAddress = `0x` + resourceID.substr(24,40);
            
            const retrievedTokenAddress = await NativeAssetHandlerInstance._resourceIDToTokenContractAddress.call(resourceID);
            assert.strictEqual(Ethers.utils.getAddress(tokenAddress).toLowerCase(), retrievedTokenAddress.toLowerCase());

            const retrievedResourceID = await NativeAssetHandlerInstance._tokenContractAddressToResourceID.call(tokenAddress);
